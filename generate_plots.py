...
# Configuration
OUTPUT_DIR = Path('research_plots')
...
print("="*80)
print("GENERATING RESEARCH PLOTS AND METRICS")
print("="*80)
...
# Load held-out test dataset (entirely separate from training data)
print("\nLoading held-out test dataset...")
test_dataset = AdaptiveRoutingDataset('data/held_out_test.jsonl')
...
checkpoint = torch.load('checkpoints/best_model.pt', weights_only=False, map_location=device)
...
bars = ax.bar(route_names, list(route_accuracies.values()), color=colors, alpha=0.8, edgecolor='black', linewidth=1.5)

ax.set_ylabel('Accuracy', fontsize=12, fontweight='bold')
ax.set_xlabel('Route Type', fontsize=12, fontweight='bold')
ax.set_title('Per-Route Classification Accuracy', fontsize=14, fontweight='bold')
...
# Extract features from held-out test dataset
features_data = []
...
try:
    ...
    ax.set_title('Privacy-Energy-Quality Trade-off\n(bubble size indicates task quality)', 
...
ax.set_title('ROC Curves for Multi-class Route Classification', fontsize=14, fontweight='bold')
...
print("\n" + "="*80)
print("ALL PLOTS AND METRICS GENERATED SUCCESSFULLY!")
print("="*80)
